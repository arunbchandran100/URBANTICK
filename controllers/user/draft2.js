...
exports.placeOrder = async (req, res) => {
    try {
        ...
        const items = cart.items.filter(
            (item) => item.product && !item.product.isBlocked
        );
        let totalAmount = 0;
        let discountPrice = 0;
        ...
        const orderItems = await Promise.all(
            items.map(async (item) => {
                ...
                if (product.stock < item.quantity) {
                    throw new Error(
                        `Insufficient stock for product ${product.product_name}. Only ${product.stock} items left.`
                    );
                }
...
exports.getOrderConfirmation = async (req, res) => {
    try {
        ...
        res
            .status(500)
            .render("error", { message: "Failed to fetch order confirmation" });
    }
};
...
exports.getUserOrders = async (req, res) => {
    try {
        const userId = req.user._id;
        const orders = await Orders.find({ "customer.customerId": userId })
            .sort({ orderDate: -1 })
...
exports.bulkOrderAction = async (req, res) => {
    try {
        ...
        res.json({
            message: `Bulk action completed. ${result.nModified} orders updated.`,
        });
    } catch (error) {
